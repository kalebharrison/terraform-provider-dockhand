...
| `dockhand_environment` | Create | `POST /api/environments`, optional `PUT /api/environments/{id}`, `POST /api/hawser/tokens` | Supports Docker environment connection + collection settings, including mTLS cert/key fields (`ca_cert`, `client_cert`, `client_key`) and `public_ip`. When create omits/ignores `publicIp`, provider follows up with PUT. `connection_type = "agent"` maps to `hawser-edge` and provisions `agent_token` through Hawser token API. | partial |
| `dockhand_environment` | Read | `GET /api/environments/{id}` | `404` removes from state. Includes `public_ip` when returned by Dockhand. | implemented |
| `dockhand_environment` | Update | `PUT /api/environments/{id}`, `POST /api/hawser/tokens` | Updates environment settings, including mTLS cert/key fields and `public_ip`. For `connection_type = "agent"`, token changes are applied through Hawser token API. | partial |
