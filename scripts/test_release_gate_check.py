...
class WorkflowGreenTest(unittest.TestCase):
    def test_workflow_green_on_main_sha_success(self) -> None:
        ...
        with mock.patch.object(gate, "workflow_runs", return_value=runs):
            self.assertTrue(gate.workflow_green_on_main_sha("Dockhand Release Watch", "abc"))

    def test_workflow_green_on_main_sha_no_match(self) -> None:
        runs = [{"status": "completed", "conclusion": "success", "headSha": "other"}]
        with mock.patch.object(gate, "workflow_runs", return_value=runs):
            self.assertFalse(gate.workflow_green_on_main_sha("Dockhand Release Watch", "abc"))

    def test_workflow_is_green_main_sha_mode(self) -> None:
        with mock.patch.object(gate, "workflow_green_on_main_sha", return_value=True):
            self.assertTrue(
    ...
    def test_workflow_is_green_strict_uses_latest(self) -> None:
        with mock.patch.object(gate, "workflow_green", return_value=False):
            self.assertFalse(
                gate.workflow_is_green(
                    gate.RELEASE_WATCH_WORKFLOW,
